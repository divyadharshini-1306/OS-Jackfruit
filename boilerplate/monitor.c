...

{
    ...
    struct monitored_entry *cursor, *next;
    ...
    list_for_each_entry_safe (cursor, next, &monitored_list, list) {

        rss_bytes = get_rss_bytes(cursor->pid);

 

        /* Case 1: The process no longer exists. Clean up. */

        if (rss_bytes < 0) {

            printk(KERN_INFO "[container_monitor] Process %d exited, cleaning up.\n",
        ...
        /* Case 2: Hard limit exceeded. Kill the process and clean up. */

        if (rss_bytes > cursor->hard_limit_bytes) {

            kill_process(cursor->container_id,
        ...
        /* Case 3: Soft limit exceeded. Warn once. */

        if (rss_bytes > cursor->soft_limit_bytes && !cursor->soft_limit_warned) {

            log_soft_limit_event(cursor->container_id,

                                 cursor->pid,

                                 cursor->soft_limit_bytes,

                                 rss_bytes);

            cursor->soft_limit_warned = 1;
...

{
    ...
    /* This must be an unregister request */
...

{
...

    {
        ...
        printk(KERN_INFO "[container_monitor] Unloading: freeing all monitored entries.\n");
